...
>(({ className, children, ...props }, ref) => {
  ...
  <GlassModalPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-[50%] top-[50%] z-50 w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] rounded-2xl backdrop-blur-sm",
        "shadow-[0_10px_40px_rgba(0,0,0,0.15)] max-h-[90vh] overflow-y-auto",
        className
      )}
      style={{
    ...
    >
      {children}
      <DialogPrimitive.Close aria-label={t('common.close') || 'Close'} className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 disabled:pointer-events-none text-foreground hover:text-primary">
        <X className="h-4 w-4" />
...
const GlassModalFooter = ({
  ...
  <div
    className={cn(
      "flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 pt-4 border-t border-glass-border/20",
      className
    )}
    {...props}
  />
