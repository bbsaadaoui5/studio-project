...
export default function StaffProfilePage() {
    ...
    const formatPaymentRate = (type?: Staff['paymentType'], rate?: number) => {
        if (!type || typeof rate !== 'number') {
            return "N/A";
        }
        switch (type) {
            case "salary":
                return new Intl.NumberFormat('ar-MA', { style: 'currency', currency: 'MAD' }).format(rate) + " / month";
            case "commission":
                return `${rate}%`;
            case "headcount":
                return new Intl.NumberFormat('ar-MA', { style: 'currency', currency: 'MAD' }).format(rate) + " / student";
            default:
                return "N/A";
        }
    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                  <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                          ...
                          <p className="font-medium capitalize">{staff?.paymentType || 'N/A'}</p>
                           <p className="text-muted-foreground">Payment Rate</p>
                          <p className="font-medium">{formatPaymentRate(staff.paymentType, staff.paymentRate)}</p>
                      </div>
                  </div>
