...
export default function ParentDashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Users /> Your Students</CardTitle>
          <CardDescription>Open the portal for each linked student.</CardDescription>
        </CardHeader>
        <CardContent>
          {students.length === 0 ? (
            <p className="text-muted-foreground">No students linked to this account yet.</p>
          ) : (
              ...
              {students.map((s) => (
                <div key={s.id} className="flex items-center justify-between border rounded p-3">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">Grade {s.grade} - {s.className}</div>
                  </div>
                  {s.link ? (
