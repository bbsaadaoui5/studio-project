...
export type PayslipItem = {
  id: string;
  label: string; // e.g., "Salaire de base", "CNSS", "Heures supplémentaires"
  amount: number;
  type: "earning" | "deduction";
  category?: string; // e.g., "base", "overtime", "tax", "insurance", "custom"
  rate?: number; // For percentage-based items (e.g., 4.29 for CNSS)
  taxable?: boolean; // Whether this item counts toward taxable income
...
export type Payslip = {
    ...
    earnings: PayslipItem[]; // All earnings including base salary, overtime, etc.
    deductions: PayslipItem[]; // All deductions (CNSS, AMO, IR, etc.)
    grossSalary: number; // Salaire brut global
    totalDeductions: number; // Total des retenues
    netPay: number;
    // Employer contributions (for reference/reporting)
    employerCNSS?: number;
    employerAMO?: number;
};
