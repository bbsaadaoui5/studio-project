...
export const addStaffMember = async (staffData: Omit<NewStaff, 'id' | 'status' | 'hireDate' | 'idNumber'> & { courseIds?: string[] }): Promise<string> => {
  ...
  // If an email is provided for teachers/admins AND a password is provided, create an auth account.
  if (role === 'teacher' || role === 'admin') {
    ...
    // If no email or no password provided, continue without creating an auth account.
  }
  ...
  // Only add paymentType and paymentRate if they're provided
  if (staffData.paymentType) {
    newStaffData.paymentType = staffData.paymentType;
  }
  if (staffData.paymentRate !== undefined && staffData.paymentRate !== null) {
    newStaffData.paymentRate = staffData.paymentRate;
  }
...
export const getStaffMembers = async (): Promise<Staff[]> => {
  ...
  if (isDevMockEnabled()) return getMockStaffMembers();
    ...
    const staffQuery = query(collection(db, "staff"), limit(500));
    const querySnapshot = await getDocs(staffQuery);
    const staff: Staff[] = [];
    ...
    console.log(`[getStaffMembers] Fetched ${staff.length} staff members from Firestore`);
    return staff;
  } catch (error) {
