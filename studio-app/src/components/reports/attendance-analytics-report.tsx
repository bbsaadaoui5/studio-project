...
function AttendanceAnalyticsReport({ filters }: AttendanceAnalyticsReportProps) {
  ...
  const filteredTopAttendees = filters && filters.grade !== "all" 
    ? topAttendees.filter(a => a.grade.toString() === filters.grade)
    : topAttendees;

  const filteredPoorAttendees = filters && filters.grade !== "all"
    ? poorAttendees.filter(a => a.grade.toString() === filters.grade)
    : poorAttendees;

  const handleExportReport = () => {
    const reportName = `attendance-report-${new Date().toISOString().split('T')[0]}.csv`;
    let csvContent = "data:text/csv;charset=utf-8,";
    csvContent += `${t('reports.attendanceReport') || 'Attendance Report'}\n`;
    csvContent += `${t('reports.generatedDate') || 'Generated Date'},${new Date().toLocaleString('ar-SA')}\n`;
    if (filters) {
      csvContent += `${t('common.grade') || 'Grade'},${filters.grade}\n`;
    }
    csvContent += "\n";
    
    // Top attendees section
    csvContent += `${t('reports.topAttendees') || 'Top Attendees'}\n`;
    csvContent += `${t('common.student') || 'Student'},${t('common.grade') || 'Grade'},${t('reports.className') || 'Class'},${t('reports.attendanceRate') || 'Attendance Rate'},${t('reports.presentDays') || 'Present Days'}/${t('reports.totalDays') || 'Total Days'}\n`;
    filteredTopAttendees.forEach(student => {
      csvContent += `${student.studentName},${student.grade},${student.className},${student.attendanceRate}%,${student.presentDays}/${student.totalDays}\n`;
    });
    
    csvContent += "\n";
    
    // Poor attendees section
    csvContent += `${t('reports.studentsNeedingAttention') || 'Students Needing Attention'}\n`;
    csvContent += `${t('common.student') || 'Student'},${t('common.grade') || 'Grade'},${t('reports.className') || 'Class'},${t('reports.attendanceRate') || 'Attendance Rate'},${t('reports.presentDays') || 'Present Days'}/${t('reports.totalDays') || 'Total Days'}\n`;
    filteredPoorAttendees.forEach(student => {
      csvContent += `${student.studentName},${student.grade},${student.className},${student.attendanceRate}%,${student.presentDays}/${student.totalDays}\n`;
    });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button variant="outline" className="text-right" onClick={handleExportReport}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {filteredTopAttendees.map((student, index) => (
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {filteredPoorAttendees.length === 0 ? (
                <p className="text-muted-foreground text-center py-4">
                  {t('reports.noStudentsWithPoorAttendance')}
                </p>
              ) : (
                filteredPoorAttendees.map((student) => (
                  <div key={student.studentId} className="flex items-center justify-between">
                    <div>
                      <p className="font-medium">{student.studentName}</p>
                      <p className="text-sm text-muted-foreground">
