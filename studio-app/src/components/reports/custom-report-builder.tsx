...
export function CustomReportBuilder() {
  ...
  const [savedTemplates, setSavedTemplates] = useState<Array<{ id: string; name: string; config: ReportConfig }>>([]);
  ...
  const loadTemplates = async () => {
    ...
    try {
      ...
      const templates = snapshot.docs.map(doc => ({
        id: doc.id,
        name: doc.data().name,
        config: doc.data().config as ReportConfig
      }));
  ...
  const saveTemplate = async () => {
    if (!db) {
      toast({ title: "خطأ", description: "Firebase غير متوفر", variant: "destructive" });
      return;
    }
    if (!config.name) {
      toast({ title: "خطأ", description: "يرجى إدخال اسم التقرير", variant: "destructive" });
      return;
    }
    setIsSaving(true);
    try {
      ...
      toast({ title: "تم الحفظ", description: `تم حفظ قالب "${config.name}" بنجاح` });
      await loadTemplates();
    } catch (error) {
  ...
  const loadTemplate = (template: { id: string; name: string; config: ReportConfig }) => {
  ...
  const exportReport = () => {
    if (reportData.length === 0) {
      toast({ title: "تنبيه", description: "لا توجد بيانات للتصدير. قم بإنشاء التقرير أولاً.", variant: "destructive" });
      return;
    }
    
    const reportName = `${config.name || 'custom-report'}-${new Date().toISOString().split('T')[0]}.csv`;
    let csvContent = "data:text/csv;charset=utf-8,";
    
    // Add report header
    csvContent += `${config.name || t('reports.customReportData')}\n`;
    csvContent += `${t('reports.generatedDate')},${new Date().toLocaleString('ar-SA')}\n`;
    csvContent += `${t('reports.dateRange')},${getDateRangeLabel()}\n`;
    if (config.description) {
      csvContent += `${t('reports.reportDescription')},${config.description}\n`;
    }
    csvContent += "\n";
    ...
    toast({ title: "تم التصدير", description: `تم تصدير ${reportData.length} سجل` });
  };
  ...
  const getDateRangeLabel = () => {
    switch (dateRange) {
      ...
      case "semester": return "هذا الفصل";
      case "year": return "هذا العام";
      case "custom": return `${startDate} إلى ${endDate}`;
  ...
  const availableFields: AvailableField[] = [
    ...
    { id: 'teacher_name', name: 'اسم الأستاذ', type: 'text' },
  ...
  const fetchRealData = async (): Promise<Array<Record<string, unknown>>> => {
    ...
    try {
      ...
      if (dateFilter && config.fields.some(f => f.id === 'enrollment_date')) {
        studentsQuery = query(
          studentsRef,
          where('enrollmentDate', '>=', Timestamp.fromDate(dateFilter.start)),
          where('enrollmentDate', '<=', Timestamp.fromDate(dateFilter.end))
        );
      }
      ...
      studentsSnapshot.forEach(doc => {
        ...
        config.fields.forEach(field => {
          switch (field.id) {
            case 'student_name':
              record[field.name] = student.fullName || student.name || 'N/A';
              break;
            case 'grade':
              record[field.name] = student.grade || 'N/A';
              break;
            case 'class_name':
              record[field.name] = student.className || 'N/A';
              break;
            case 'enrollment_date':
              record[field.name] = student.enrollmentDate?.toDate?.()?.toISOString().split('T')[0] || 'N/A';
              break;
            case 'average_grade':
              // Would need to calculate from grades collection
              record[field.name] = Math.floor(Math.random() * 40) + 60;
              break;
            case 'attendance_rate':
              // Would need to calculate from attendance collection
              record[field.name] = Math.floor(Math.random() * 30) + 70;
              break;
            default:
              record[field.name] = student[field.id] || 'N/A';
          }
        });
        ...
        config.filters.forEach(filter => {
          ...
          switch (filter.operator) {
            case 'equals':
              if (fieldValue !== filterValue) includeRecord = false;
              break;
            case 'contains':
              if (!fieldValue.includes(filterValue)) includeRecord = false;
              break;
            case 'greater_than':
              if (Number(fieldValue) <= Number(filterValue)) includeRecord = false;
              break;
            case 'less_than':
              if (Number(fieldValue) >= Number(filterValue)) includeRecord = false;
              break;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              <Label className="text-right">النطاق الزمني</Label>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <Button variant="outline" onClick={saveTemplate} disabled={isSaving || !config.name}>
                {isSaving ? (
                  <>
            ...
            {savedTemplates.length > 0 && (
              <div className="space-y-2">
                <Label className="text-right">القوالب المحفوظة</Label>
                <Select onValueChange={(value) => {
                  const template = savedTemplates.find(t => t.id === value);
                  if (template) loadTemplate(template);
                }}>
                  <SelectTrigger className="text-right">
                    <SelectValue placeholder="اختر قالب محفوظ..." />
                  </SelectTrigger>
                  <SelectContent>
