...
describe('getCombinedMonthlyDueForStudent', () => {
  ...
  test('returns grade monthly when no support courses', async () => {
    ...
    getStudentMock.mockResolvedValueOnce({ id: 'stu1', grade: '5', enrollmentDate: '2024-09-01' });
    getCoursesForStudentMock.mockResolvedValueOnce([]);
  ...
  test('aggregates support course monthly fees and adds to grade fee', async () => {
    getStudentMock.mockResolvedValueOnce({ id: 'stu2', grade: '6', enrollmentDate: '2024-09-01' });
    getCoursesForStudentMock.mockResolvedValueOnce(['c1', 'c2']);
    ...
    getCourseMock.mockImplementation(async (id: string) => {
  ...
  test('skips grade fee when grade is N/A and still sums support fees', async () => {
    getStudentMock.mockResolvedValueOnce({ id: 'stu3', grade: 'N/A' });
    getCoursesForStudentMock.mockResolvedValueOnce(['c3']);
    ...
    getCourseMock.mockResolvedValueOnce({ id: 'c3', type: 'support', monthlyFee: 45, name: 'Support C', teachers: [], description: '', credits: 0, department: '', grade: '', createdAt: new Date().toISOString() } as Course);
